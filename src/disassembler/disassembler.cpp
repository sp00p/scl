...
std::vector<DisassembledInstruction> Disassembler::disassemble(const std::vector<uint8_t>& rom) {
    ...
    // Walk the ROM following the code-flow analysis rather than assuming
    // 2-byte alignment: CHIP-8 allows odd-aligned code, and data regions may
    // shift instruction boundaries.
    size_t i = 0;
    while (i < rom.size()) {
        DisassembledInstruction instr;
        instr.address = 0x200 + static_cast<uint16_t>(i);

        if (is_code[i] && i + 1 < rom.size()) {
            ...
            i += 2;
        } else if (i + 1 < rom.size() && !is_code[i + 1]) {
            // Two consecutive data bytes: emit as a word
            instr.opcode = (rom[i] << 8) | rom[i + 1];
            instr.is_data = true;
            std::ostringstream ss;
            ...
            // Single data byte (next byte is code or end of ROM)
            instr.opcode = rom[i];
            instr.is_data = true;
            std::ostringstream ss;
            ss << "DB 0x" << std::hex << std::uppercase << std::setfill('0') << std::setw(2) << (int)rom[i];
            instr.mnemonic = ss.str();
            instr.comment = "; data";
            i += 1;
        }
...
void Disassembler::output(std::ostream& out, const std::vector<DisassembledInstruction>& instructions,
    ...
    for (const auto& instr : instructions) {
        // std::left set for the mnemonic column below is sticky; reset it so
        // zero-padded hex fields pad on the left as intended
        out << std::right;
        ...
        if (show_bytes) {
            // Single data bytes print as 2 hex digits, opcodes as 4
            int width = (instr.is_data && instr.opcode <= 0xFF && instr.mnemonic.substr(0, 2) == "DB") ? 2 : 4;
            out << std::hex << std::uppercase << std::setfill('0') << std::setw(width) << instr.opcode
                << std::string(6 - width, ' ');
        }
