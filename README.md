...
If you'd like to try this out yourself you're probably going to have to recompile the binary (compiled in mac will add proper archives later). The current tests the work are the chip8-test-suite, the IBM Logo, and the BC_test. To change the test used, edit the line in load_rom where it assigns the memory address 0x1FF to a number. For now, numbers between 1-3 work as expected. Until user input is added, tests 4 and 5 will NOT work.
